...
```
curl https://raw.githubusercontent.com/Arcadia-Science/peptigate/main/scripts/characterize_peptides.py?token=GHSAT0AAAAAACPE43ZXXIARD4XPPPE2JMBWZPHWYPA
...
9. Check that the data set is predictive
```
library(caret)
...
final_dataset_filtered <- final_dataset %>%
  select(-rep, -member_sequence, -member, -all_bioactivities, -go_id, -pfam_id,
         -pfam_name, -pfam_type)
...
* `rep`: representative peptide id for the cluster. Generated by running MMSeqs2 `linclust` on the peptipedia database with an identity threshold of 30%.
* `member`: cluster member peptide id.
* `member_sequence`: cluster member peptide sequence. 
